...
class CommandsCog(commands.Cog):
    ...
    @commands.command(brief=utils.translate('command-setup-brief', 'http://g5.thboss.xyz:3301'),
    ...
    async def setup(self, ctx, api_user_id=None, api_user_key=None):
        ...
        if not api_user_id or not api_user_key:
            msg = utils.translate('invalid-usage', self.bot.command_prefix[0], ctx.command.usage)
            raise commands.UserInputError(message=msg)

        is_user = await self.bot.api.is_user(int(api_user_id))
        if not is_user:
            msg = utils.translate('command-setup-key-invalid')
            raise commands.UserInputError(message=msg)
        ...
        guild_data = await utils.get_guild_data(self.bot, ctx.guild.id)
    ...
    async def create_server(self, ctx, *args):
        ...
        if not result:
            msg = utils.translate('command-create_server-failed')
            raise commands.UserInputError(message=msg)

        msg = utils.translate('command-create_server-success', ip, port)
        embed = self.bot.embed_template(title=msg, color=self.bot.colors['green'])
        await ctx.send(embed=embed)

    @commands.command(usage='lobby <name>',
                      brief=utils.translate('command-lobby-brief'))
    @commands.has_permissions(kick_members=True)
    async def lobby(self, ctx, *args):
        ...
        msg = utils.translate('command-lobby-success', args)
        embed = self.bot.embed_template(title=msg, color=self.bot.colors['green'])
        await ctx.send(embed=embed)

    @commands.command(usage='link <Steam ID/Profile>',
                      brief=utils.translate('command-link-brief'))
    async def link(self, ctx, *args):
        """"""
        guild_data = await utils.check_setup(self.bot, ctx)
        user_data = await utils.get_user_data(self.bot, ctx.guild, ctx.author.id)
        banned_users = await self.bot.db.get_banned_users(ctx.guild.id)

        if ctx.author.id in banned_users:
            msg = utils.translate('command-link-user-is-banned')
            unban_time = banned_users[ctx.author.id]
            if unban_time:
                msg += f' for {utils.timedelta_str(unban_time - datetime.now(timezone.utc))}'
            raise commands.UserInputError(message=msg)

        if user_data is not None:
            msg = utils.translate('command-link-already-linked', user_data.steam)
        ...
        if not steam_id.is_valid():
            ...
            if steam_id is None:
                ...
                if steam_id is None:
                    msg = utils.translate('command-link-steam-invalid')
                    raise commands.UserInputError(message=msg)

        user_data = await utils.get_user_data(self.bot, ctx.guild, str(steam_id), 'steam_id')
        if user_data is not None:
            msg = utils.translate('command-link-steam-used')
            raise commands.UserInputError(message=msg)

        await self.bot.db.insert_users(ctx.author.id, str(steam_id), os.environ['GET5_CAPTAIN_FLAG'])
        await ctx.author.add_roles(guild_data.linked_role)

        title = utils.translate('command-link-success', steam_id)
        embed = self.bot.embed_template(description=title, color=self.bot.colors['green'])
        embed.set_footer(text=utils.translate('command-link-footer'))
        await ctx.send(embed=embed)

    @commands.command(brief=utils.translate('command-unlink-brief'))
    async def unlink(self, ctx):
        ...
        awaitables = [
            utils.get_user_data(self.bot, ctx.guild, ctx.author.id),
        ...
        if not is_linked:
            msg = utils.translate('command-unlink-not-linked')
            raise commands.UserInputError(message=msg)

        if ctx.author.id in queued_ids:
            msg = utils.translate('command-unlink-in-lobby')
            raise commands.UserInputError(message=msg)

        if ctx.author.id in matches_users:
            msg = utils.translate('command-unlink-in-match')
            raise commands.UserInputError(message=msg)

        await self.bot.db.delete_users([ctx.author.id])
        await ctx.author.remove_roles(guild_data.linked_role)

        title = utils.translate('command-unlink-success', ctx.author)
        embed = self.bot.embed_template(title=title, color=self.bot.colors['green'])
        embed.set_footer(text=utils.translate('command-unlink-footer'))
        await ctx.send(embed=embed)

    @commands.command(usage='empty <mention_queue_channel>',
                      brief=utils.translate('command-empty-brief'))
    @commands.has_permissions(kick_members=True)
    ...
    @commands.command(usage='cap <mention_queue_channel> <new_capacity>',
                      brief=utils.translate('command-cap-brief'),
                      aliases=['capacity'])
    @commands.has_permissions(kick_members=True)
    async def cap(self, ctx):
        ...
        if new_cap == curr_cap:
            msg = utils.translate('command-cap-already', curr_cap)
            raise commands.UserInputError(message=msg)

        if new_cap < 2 or new_cap > 100 or new_cap %2 == 1:
            msg = utils.translate('command-cap-out-range')
            raise commands.UserInputError(message=msg)

        if self.lobby_cog.locked_lobby[pug_data.id]:
            msg = utils.translate('command-cap-locked')
            raise commands.UserInputError(message=msg)

        self.lobby_cog.locked_lobby[pug_data.id] = True
        await self.bot.db.clear_queued_users(pug_data.id)
        await self.bot.db.update_pug(pug_data.id, capacity=new_cap)
        embed = await self.lobby_cog.queue_embed(pug_data, utils.translate('command-empty-success'))
        embed.set_footer(text=utils.translate('command-cap-footer'))
        await self.lobby_cog.update_last_msg(pug_data, embed)
        msg = utils.translate('command-cap-success', new_cap)
        lobby_channel = pug_data.lobby_channel
        guild_data = await utils.get_guild_data(self.bot, ctx.guild.id)
    ...
    @commands.command(usage='teams <mention_queue_channel> <captains|autobalance|random>',
                      brief=utils.translate('command-teams-brief'),
                      aliases=['team'])
    @commands.has_permissions(kick_members=True)
    async def teams(self, ctx):
        ...
        if method is None:
            title = utils.translate('command-teams-method', curr_method)
        else:
            if method == curr_method:
                msg = utils.translate('command-teams-already', curr_method)
                raise commands.UserInputError(message=msg)

            if method not in valid_methods:
                msg = utils.translate('command-teams-invalid', valid_methods[0], valid_methods[1])
                raise commands.UserInputError(message=msg)

            title = utils.translate('command-teams-changed', method)
    ...
    @commands.command(usage='captains <mention_queue_channel> <volunteer|rank|random>',
                      brief=utils.translate('command-captains-brief'),
    ...
    async def captains(self, ctx):
        ...
        if new_method is None:
            title = utils.translate('command-captains-method', curr_method)
        else:
            if new_method == curr_method:
                msg = utils.translate('command-captains-already', curr_method)
                raise commands.UserInputError(message=msg)

            if new_method not in valid_methods:
                msg = utils.translate('command-captains-invalid', valid_methods[0], valid_methods[1])
                raise commands.UserInputError(message=msg)

            title = utils.translate('command-captains-changed', new_method)
    ...
    @commands.command(usage='maps <mention_queue_channel> <ban|vote|random>',
                      brief=utils.translate('command-maps-brief'),
                      aliases=['map'])
    @commands.has_permissions(kick_members=True)
    async def maps(self, ctx):
        ...
        if new_method is None:
            title = utils.translate('command-maps-method', curr_method)
        else:
            if new_method == curr_method:
                msg = utils.translate('command-maps-already', curr_method)
                raise commands.UserInputError(message=msg)

            if new_method not in valid_methods:
                msg = utils.translate('command-maps-invalid', valid_methods[0], valid_methods[1], valid_methods[2])
                raise commands.UserInputError(message=msg)

            title = utils.translate('command-maps-changed', new_method)
    ...
    @commands.command(usage='mpool <mention_queue_channel>',
                      brief=utils.translate('command-mpool-brief'),
    ...
    @commands.command(usage='spectators <mention_queue_channel> {+|-} <mention> <mention> ...',
                      brief=utils.translate('command-spectators-brief'),
        ...
        for spectator in spectators:
            if prefix == '+':
                if spectator.id in curr_spectator_ids:
                    msg = f'{utils.translate("command-spectators-already", spectator.display_name)}'
                    raise commands.UserInputError(message=msg)

                await self.bot.db.insert_spect_users(pug_data.id, spectator.id)
                title += f'{utils.translate("command-spectators-added", spectator.display_name)}\n'

            elif prefix == '-':
                if spectator.id not in curr_spectator_ids:
                    msg = f'{utils.translate("command-spectators-not", spectator.display_name)}'
                    raise commands.UserInputError(message=msg)

                await self.bot.db.delete_spect_users(pug_data.id, spectator.id)
                title += f'{utils.translate("ommand-spectators-removed", spectator.display_name)}\n'

        embed = self.bot.embed_template(title=title)
        await ctx.send(embed=embed)

    @commands.command(usage='end <match_id>',
                      brief=utils.translate('command-end-brief'),
                      aliases=['cancel', 'stop'])
    @commands.has_permissions(kick_members=True)
    async def end(self, ctx, match_id=None):
        ...
        if status_code != 200:
            if status_code == 404:
                msg = utils.translate('command-end-not-found', match_id)
            elif status_code == 403:
                msg = utils.translate('command-end-no-permission')
            elif status_code == 401:
                msg = utils.translate('command-end-already-finished', match_id)
            else:
                msg = utils.translate('command-end-unknown-error')
            raise commands.UserInputError(message=msg)

        title = utils.translate('command-end-success', match_id)
        embed = self.bot.embed_template(title=title)
        await ctx.send(embed=embed)

    @commands.command(brief=utils.translate('command-stats-brief'),
                      aliases=['rank'])
    async def stats(self, ctx):
        ...
        description = '```ml\n' \
                     f' {utils.translate("command-stats-kills")}:             {stats.kills} \n' \
                     f' {utils.translate("command-stats-deaths")}:            {stats.deaths} \n' \
                     f' {utils.translate("command-stats-assists")}:           {stats.assists} \n' \
                     f' {utils.translate("command-stats-kdr")}:         {stats.kdr} \n' \
                     f' {utils.translate("command-stats-hs")}:         {stats.hsk} \n' \
                     f' {utils.translate("command-stats-hsp")}:  {stats.hsp} \n' \
                     f' {utils.translate("command-stats-played")}:    {stats.total_maps} \n' \
                     f' {utils.translate("command-stats-wins")}:        {stats.wins} \n' \
                     f' {utils.translate("command-stats-win-rate")}:       {stats.win_percent} \n' \
                     f' ------------------------- \n' \
                     f' {utils.translate("command-stats-rating")}:    {stats.average_rating} \n' \
                      '```'
    ...
    @commands.command(brief=utils.translate('command-leaders-brief'),
                      aliases=['top', 'ranks'])
    async def leaders(self, ctx):
        ...
        # Send leaderboard
        title = f'__{utils.translate("command-leaders-leaderboard")}__'
    ...
    @commands.command(usage='ban <user mention> ... [<days>d] [<hours>h] [<minutes>m]',
                      brief=utils.translate('command-ban-brief'),
                      aliases=['banned'])
    ...
    async def ban(self, ctx, *args):
        ...
        if len(ctx.message.mentions) == 0:
            msg = utils.translate('command-ban-mention-to-ban')
            raise commands.UserInputError(message=msg)
        ...
        embed.set_footer(text=utils.translate('command-ban-footer'))
        await ctx.send(embed=embed)
    ...
    @commands.command(usage='unban <user mention> ...',
                      brief=utils.translate('command-unban-brief'),
                      aliases=['unbanned'])
    @commands.has_permissions(ban_members=True)
    async def unban(self, ctx):
        ...
        guild_data = await utils.check_setup(self.bot, ctx)
        if len(ctx.message.mentions) == 0:
            msg = utils.translate('command-unban-mention-to-unban')
            raise commands.UserInputError(message=msg)
        ...
        embed.set_footer(text=utils.translate('command-unban-footer'))
        await ctx.send(embed=embed)
    ...
    @commands.command(usage='add <match_id> <team1|team2|spec> <mention>',
                      brief=utils.translate('command-add-brief'))
    @commands.has_permissions(kick_members=True)
    async def add(self, ctx, match_id=None, team=None):
        ...
        user_data = await utils.get_user_data(self.bot, ctx.guild, user.id)
        if not user_data:
            msg = utils.translate('command-add-not-linked', user.mention)
            raise commands.UserInputError(message=msg)
        ...
        if status_code != 200:
            if status_code == 404:
                msg = utils.translate('command-add-match-not-exist', match_id)
            elif status_code == 403:
                msg = utils.translate('command-add-no-permission')
            elif status_code == 401:
                msg = utils.translate('command-add-already-finished', match_id)
            elif status_code == 500:
                msg = utils.translate('command-add-game-server-error')
            else:
                msg = utils.translate('command-add-unknown-error')
            raise commands.UserInputError(message=msg)
        ...
        msg = utils.translate('command-add-success', user.mention, match_id)
        embed = self.bot.embed_template(description=msg)
        await ctx.send(embed=embed)

    @commands.command(usage='remove <match_id> <mention>',
                      brief=utils.translate('command-remove-brief'))
    @commands.has_permissions(kick_members=True)
    async def remove(self, ctx, match_id=None):
        ...
        if status_code != 200:
            if status_code == 404:
                msg = utils.translate('command-remove-match-not-exist', match_id)
            elif status_code == 403:
                msg = utils.translate('command-remove-no-permission')
            elif status_code == 401:
                msg = utils.translate('command-remove-already-finished', match_id)
            elif status_code == 500:
                msg = utils.translate('command-remove-game-server-error')
            else:
                msg = utils.translate('command-remove-unknown-error')
            raise commands.UserInputError(message=msg)
        ...
        msg = utils.translate('command-remove-success', user.mention, match_id)
        embed = self.bot.embed_template(description=msg)
        await ctx.send(embed=embed)

    @commands.command(usage='pause <match_id>',
                      brief=utils.translate('command-pause-brief'))
    @commands.has_permissions(kick_members=True)
    async def pause(self, ctx, match_id=None):
        ...
        if status_code != 200:
            if status_code == 404:
                msg = utils.translate('command-pause-not-found', match_id)
            elif status_code == 403:
                msg = utils.translate('command-pause-no-permission')
            elif status_code == 401:
                msg = utils.translate('command-pause-already-finished', match_id)
            else:
                msg = utils.translate('command-pause-unknown-error')
            raise commands.UserInputError(message=msg)

        msg = utils.translate('command-pause-success', match_id)
        embed = self.bot.embed_template(description=msg)
        await ctx.send(embed=embed)

    @commands.command(usage='unpause <match_id>',
                      brief=utils.translate('command-unpause-brief'))
    @commands.has_permissions(kick_members=True)
    async def unpause(self, ctx, match_id=None):
        ...
        if status_code != 200:
            if status_code == 404:
                msg = utils.translate('command-unpause-not-found', match_id)
            elif status_code == 403:
                msg = utils.translate('command-unpause-no-permission')
            elif status_code == 401:
                msg = utils.translate('command-unpause-already-finished', match_id)
            else:
                msg = utils.translate('command-unpause-unknown-error')
            raise commands.UserInputError(message=msg)

        msg = utils.translate('command-unpause-success', match_id)
        embed = self.bot.embed_template(description=msg)
        await ctx.send(embed=embed)
    ...
    async def config_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingPermissions):
            ...
            embed = self.bot.embed_template(title=utils.translate('command-required-perm', missing_perm),
